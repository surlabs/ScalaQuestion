...
/**
 * SCALA Question EXPORT OF QUESTIONS to ILIAS
 *
...
class assScalaQuestionExport extends assQuestionExport
{
	/** @var assScalaQuestion */
	var $object;
	...
	function toXML($a_include_header = true, $a_include_binary = true, $a_shuffle = false, $test_output = false, $force_image_references = false): string
	{
		...
		//get Question Array
		$question_array = $this->object->getScala()->getRawData();
		...
		//QUESTION
		$a_xml_writer->xmlStartTag("qtimetadatafield");
		$a_xml_writer->xmlElement("fieldlabel", NULL, "scala_question");
	 ...
	 */
	private static function _getRTEMedia($a_text, $scala_question = ""): array
	{
		$media = array();
		$matches = array();
